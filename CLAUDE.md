...
**Important right now:** the project is mid-expansion from its original 36-vehicle demo into full India-market coverage, one small OEM batch at a time (see `HANDOFF.md`'s "Full-Market Expansion — Batch Log"). **Batches 1-3 are done** — `src/lib/data/cars.ts` has 54 records across 15 car OEMs (Tata, Mahindra, MG, Hyundai, Kia, BYD, BMW, Mercedes-Benz, Audi, Volvo, MINI, Porsche, Lotus, Rolls-Royce, VinFast); `src/lib/data/two-wheelers.ts` has 54 records across 23 two-wheeler OEMs (the full Batch 3 scooter brand list). Motorcycles (Batch 4) and commercial EVs (Batch 5) are not started — **do not start a new batch without being asked**, each batch stops for approval when done. Separately, the commercial-category *architecture* is complete and working; the commercial *data* is not — `src/lib/data/commercial.ts` is an empty array. `src/lib/data/_research-commercial.ts` (earmarked for Batch 5) and `src/lib/data/_research-two-wheelers.ts` (now trimmed to just Ultraviolette/Revolt motorcycle data, earmarked for Batch 4) both still contain real, researched-but-unmerged data. See `HANDOFF.md`'s "Known Limitations" and "How the next Claude session should continue" before doing anything with the vehicle database.
...
7. **Quality gate, every time:** `npx tsc --noEmit`, `npx eslint .`, `npm run build` — all three are clean as of this handoff (157 routes). Keep them clean.
...
Commercial-EV category has real architecture but zero data (see above). Two research staging files (`_research-commercial.ts` for Batch 5, `_research-two-wheelers.ts` now trimmed to Ultraviolette/Revolt for Batch 4) contain real unmerged vehicle data. Car dataset covers Tata/Mahindra/MG/Hyundai/Kia (Batch 1) plus BYD/BMW/Mercedes-Benz/Audi/Volvo/MINI/Porsche/Lotus/Rolls-Royce/VinFast (Batch 2) — 54 records, 15 car OEMs. Two-wheeler dataset covers all 22 Batch-3 scooter brands plus Okinawa — 54 records, 23 two-wheeler OEMs. Motorcycles/commercial EVs are not started (Batches 4-5). Price display doesn't switch to crore notation for the new 100L+ luxury cars (cosmetic, flagged not fixed). Multi-word search queries that skip a word (e.g. "simple ultra") return no results — pre-existing `search.ts` characteristic, not a regression. "Portable Battery" isn't a filterable field despite being requested for Batch 3 — needs a schema change, flagged not done. Ad slots are static placeholders, no pagination yet, Reviews/Latest News are honest empty states with working forms behind them, no real backend, placeholder SEO domain, a few shadcn primitives left unrestyled (`Tabs`, `Popover`/`Command`). Full reasoning for each is in `HANDOFF.md`'s Known Limitations section.
