...
src/
  ...
  lib/
    data/                       cars.ts (54), two-wheelers.ts (68 — see Batch Log), oems.ts (46), commercial.ts (0 — see Known Limitations) + index.ts (getOemBySlug, getVehiclesByOem, getVehiclesByCategory, getAllVehicles, getVehicleBySlug)
...
- **The full-market vehicle-data expansion is underway.** Batches 1-4 are done — see [Full-Market Expansion — Batch Log](#full-market-expansion--batch-log). `cars.ts` went from 18 to 54 records across 15 car OEMs; `two-wheelers.ts` went from 18 to 68 records (54 scooters + 14 motorcycles) across 31 two-wheeler OEMs. All live-verified in routing, search, listings, brand pages, and compare, with a clean `tsc`/`eslint`/`build` gate (179 routes, up from 59 pre-expansion). Commercial EVs (Batch 5) is not started.
- **Nothing described in this file is committed to git except after this session's and the prior sessions' commits.** Before Batch 1, `git log` showed the same 5 commits as the original QA cycle, with two full sessions of location/filter/search and category-architecture work sitting uncommitted. Batches 1 through 4 each commit their own scoped data work — check `git log`/`git status` to see exactly what landed vs. what's still pending from before.

As a demonstration of what a polished, fully data-driven, multi-category EV marketplace's *architecture* looks like, this is in good shape — it now spans everything from a ₹0.6L licence-free scooter to a ₹9.5Cr Rolls-Royce in the same filter/search/compare system without a single line of category-specific page code. As an actual marketplace with real inventory, it's substantially further along on cars and two-wheelers (54 and 68 records respectively, covering mainstream through luxury on the car side and scooters plus motorcycles across 31 brands on the two-wheeler side) but not yet started on commercial EVs.
...
3. **If continuing the full-market expansion, the next step is Batch 5** (commercial EVs — Tata, Mahindra, Euler, Omega Seiki, Piaggio, Altigreen, Montra, Switch, Ashok Leyland, and any other officially available commercial EV per the original batch plan) — see the [Batch Log](#full-market-expansion--batch-log) for the exact working method (sequential research, one model per record, integrate+verify+quality-gate+commit before moving on, stop after the batch for approval). **Do not start Batch 5 without being asked** — Batch 4 was explicitly scoped to stop and wait. `src/lib/data/_research-commercial.ts` (already present, uncommitted) contains real researched-but-unmerged data that may accelerate this batch — spot-check before merging, same as every prior batch. Note `src/lib/data/commercial.ts` currently exports an empty array, so this batch is the first to give the already-complete commercial-category architecture (see [Architecture overview](#architecture-overview)) any actual data.
4. **Before calling any future batch done, re-check `vehicle-filter-options.ts`'s `priceBounds`/`rangeBounds`/`batteryBounds`** against the new dataset's actual min/max — for whichever category's `CategoryFilterConfig` you're adding to (`CAR_FILTER_CONFIG`, `TWO_WHEELER_FILTER_CONFIG`, or `COMMERCIAL_FILTER_CONFIG`). Batch 1 hit this bug live (Ioniq 9 exceeded the old car price/battery bounds and silently vanished from listings); Batches 2-4 each caught and fixed similar bound overflows *before* declaring the batch done (Batch 2: Rolls-Royce Spectre needed the car price bound raised to ₹950L; Batch 3: Simple Energy Ultra needed the two-wheeler range/battery bounds raised; Batch 4: Ultraviolette's F77 needed the two-wheeler price bound raised to ₹4.5L and battery bound to 11kWh) — each time confirming the exact vehicle count on first load rather than discovering the bug after the fact. Neither typechecking nor the build catches this — only live verification of the listing count does. `COMMERCIAL_FILTER_CONFIG`'s current bounds (spanning "a ~1.5L cargo 3-wheeler to a ~2.5 Cr electric bus," per its own code comment) were set speculatively before any commercial data existed — check them carefully once Batch 5 adds real records.
5. **`src/lib/data/_research-two-wheelers.ts` no longer exists.** Everything it ever staged (scooters merged in Batch 3, motorcycles merged in Batch 4) is now in `two-wheelers.ts`/`oems.ts`, and the file was deleted at the end of Batch 4. Don't go looking for it.
