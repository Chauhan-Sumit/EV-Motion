...
**Last updated:** 2026-08-01 (full-market-expansion Batch 1 complete)
**Status:** Feature-complete demo marketplace (cars + two-wheelers) with a production-readiness QA cycle behind it, mid-expansion into a fully data-driven, multi-category EV marketplace (cars, two-wheelers, and a new commercial-EV category), and now also mid-expansion of the **car dataset itself** into full India-market coverage, working one OEM group at a time. **Batch 1 (Tata, Mahindra, MG, Hyundai, Kia) is complete** — see [Full-Market Expansion — Batch Log](#full-market-expansion--batch-log). The category *architecture* (commercial EVs, N-category system) is complete and verified; the commercial *data* is not — see [Known Limitations](#known-limitations) and [Next Session Instructions](#how-the-next-claude-session-should-continue). Nothing described below is committed to git yet except the original QA-cycle work — see [Project Status](#project-status).
...
src/
  ...
  lib/
    data/                       cars.ts (29 — see Batch Log), two-wheelers.ts (18), oems.ts (12), commercial.ts (0 — see Known Limitations) + index.ts (getOemBySlug, getVehiclesByOem, getVehiclesByCategory, getAllVehicles, getVehicleBySlug)
...
### Batch 2 — BYD, BMW, Mercedes-Benz, Audi, Volvo, Mini, Porsche, Lotus, Rolls-Royce, VinFast + other luxury EV brands (cars) — not started

BYD already has 3 records in `cars.ts` (Atto 3, Seal, e6) and an OEM entry — Batch 2 should audit whether BYD's lineup is complete (e.g. Seal U/eMAX 7/Sealion, if officially launched in India) before moving to the untouched luxury brands. Do not start this until told to — Batch 1 stops here for approval per the working method.
...
- **The car dataset is mid-expansion, not yet complete.** Batch 1 (Tata, Mahindra, MG, Hyundai, Kia) is done — `cars.ts` now has 29 records. BYD and every luxury EV brand (BMW, Mercedes-Benz, Audi, Volvo, Mini, Porsche, Lotus, Rolls-Royce, VinFast, etc.) are Batch 2, not yet started.
...
- **No pagination on listing pages** — the catalog is still small enough (29-18-0) that a single page is correct. Revisit once any category's real count grows past roughly 30-40 (cars is now approaching that with more batches still to come).
...
- **The full-market car-data expansion is underway.** Batch 1 (Tata, Mahindra, MG, Hyundai, Kia) is done — see [Full-Market Expansion — Batch Log](#full-market-expansion--batch-log). `cars.ts` went from 18 to 29 records, all live-verified in routing, search, listings, brand pages, and compare, with a clean `tsc`/`eslint`/`build` gate (70 routes, up from 59). BYD/luxury brands (Batch 2), two-wheelers (Batch 3), motorcycles (Batch 4), and commercial EVs (Batch 5) are not started.
- **Nothing described in this file is committed to git except after this session's commit.** Before this session, `git log` showed the same 5 commits as the original QA cycle, with two full sessions of location/filter/search and category-architecture work sitting uncommitted. This session commits its own Batch 1 data work — check `git log`/`git status` to see exactly what landed vs. what's still pending from before.

As a demonstration of what a polished, fully data-driven, multi-category EV marketplace's *architecture* looks like, this is in good shape. As an actual marketplace with real inventory, it's partway there on cars (29 of an eventual much larger list) and not yet started on commercial EVs or the two-wheeler/motorcycle expansion.
...
3. **If continuing the full-market car expansion, the next step is Batch 2** (BYD lineup audit + BMW, Mercedes-Benz, Audi, Volvo, Mini, Porsche, Lotus, Rolls-Royce, VinFast, and any other officially-selling luxury EV brand) — see the [Batch Log](#full-market-expansion--batch-log) for the exact working method (sequential research, one model per record, integrate+verify+quality-gate+commit before moving on, stop after the batch for approval). **Do not start Batch 2 without being asked** — Batch 1 was explicitly scoped to stop and wait.
4. **Before calling any future batch done, re-check `vehicle-filter-options.ts`'s `priceBounds`/`rangeBounds`/`batteryBounds`** against the new dataset's actual min/max. Batch 1 hit this bug live: Ioniq 9 exceeded the old `[0, 90]` price bound and `[0, 100]` battery bound, silently vanishing from `/cars` and the homepage filter chips despite a clean `tsc`/`eslint`/`build`. Neither typechecking nor the build catches this — only live verification of the listing count does.
