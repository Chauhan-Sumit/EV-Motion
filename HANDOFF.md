...
src/
  ...
  lib/
    data/                       cars.ts (54 — see Batch Log), two-wheelers.ts (18), oems.ts (21), commercial.ts (0 — see Known Limitations) + index.ts (getOemBySlug, getVehiclesByOem, getVehiclesByCategory, getAllVehicles, getVehicleBySlug)
...
Two-wheelers (Batch 3), motorcycles (Batch 4), commercial EVs (Batch 5) per the original batch plan. Note `src/lib/data/_research-two-wheelers.ts` and `src/lib/data/_research-commercial.ts` (both still present, untouched, uncommitted) already contain real researched-but-unmerged data from a prior interrupted session that may accelerate Batches 3 and 5 — spot-check before merging, per the existing guidance below.
...
- **Two uncommitted files contain real, researched-but-unmerged vehicle data**: `src/lib/data/_research-commercial.ts` (~30 commercial EV records + new OEMs) and `src/lib/data/_research-two-wheelers.ts` (~30 additional two-wheeler records + new OEMs), both sourced via web research in a prior interrupted session. Neither has been spot-checked for accuracy or merged into the real data files yet — earmarked for Batches 3 and 5 of the [full-market expansion](#full-market-expansion--batch-log).
- **The car dataset is mid-expansion, not yet complete.** Batches 1 (Tata, Mahindra, MG, Hyundai, Kia) and 2 (BYD audit + BMW, Mercedes-Benz, Audi, Volvo, MINI, Porsche, Lotus, Rolls-Royce, VinFast) are done — `cars.ts` now has 54 records. Two-wheelers/motorcycles (Batches 3–4) and commercial EVs (Batch 5) are not started.
...
- **A few researched-but-uncertain models were deliberately left out of Batch 2**: Audi Q6 e-tron, Volvo EX90/ES90, MINI Aceman, VinFast VF9/VF3/Limo Green (all announced-but-unlaunched with unreliable India-spec sourcing), and Polestar (India availability itself unconfirmed). Worth a second pass once official numbers land — see the Batch 2 log for detail.
...
- **No pagination on listing pages** — the catalog is still small enough (54-18-0) that a single page is correct. Revisit once any category's real count grows past roughly 60-80 (cars is now well past the original 30-40 estimate with more batches still to come).
...
- **The full-market car-data expansion is underway.** Batches 1 and 2 are done — see [Full-Market Expansion — Batch Log](#full-market-expansion--batch-log). `cars.ts` went from 18 to 54 records across 15 car OEMs, all live-verified in routing, search, listings, brand pages, and compare, with a clean `tsc`/`eslint`/`build` gate (104 routes, up from 59 pre-expansion). Two-wheelers (Batch 3), motorcycles (Batch 4), and commercial EVs (Batch 5) are not started.
- **Nothing described in this file is committed to git except after this session's and the prior session's commits.** Before Batch 1, `git log` showed the same 5 commits as the original QA cycle, with two full sessions of location/filter/search and category-architecture work sitting uncommitted. Batches 1 and 2 each commit their own scoped data work — check `git log`/`git status` to see exactly what landed vs. what's still pending from before.

As a demonstration of what a polished, fully data-driven, multi-category EV marketplace's *architecture* looks like, this is in good shape — it now spans everything from a ₹6.99L city hatchback to a ₹9.5Cr Rolls-Royce in the same filter/search/compare system without a single line of category-specific page code. As an actual marketplace with real inventory, it's substantially further along on cars (54 of an eventual larger list, covering mainstream + luxury) but not yet started on commercial EVs or the two-wheeler/motorcycle expansion.
...
3. **If continuing the full-market car expansion, the next step is Batch 3** (two-wheelers — Ola, Ather, TVS, Hero Vida, Bajaj, River, Simple, Ampere, BGauss, Bounce, Quantum, Okaya, Odysse, Zelio, Kinetic, Lectrix, and every other officially active scooter manufacturer per the original batch plan) — see the [Batch Log](#full-market-expansion--batch-log) for the exact working method (sequential research, one model per record, integrate+verify+quality-gate+commit before moving on, stop after the batch for approval). **Do not start Batch 3 without being asked** — Batch 2 was explicitly scoped to stop and wait. `src/lib/data/_research-two-wheelers.ts` (already present, uncommitted) may accelerate this — spot-check before merging.
4. **Before calling any future batch done, re-check `vehicle-filter-options.ts`'s `priceBounds`/`rangeBounds`/`batteryBounds`** against the new dataset's actual min/max — for whichever category's `CategoryFilterConfig` you're adding to (`CAR_FILTER_CONFIG`, `TWO_WHEELER_FILTER_CONFIG`, or `COMMERCIAL_FILTER_CONFIG`). Batch 1 hit this bug live (Ioniq 9 exceeded the old car price/battery bounds and silently vanished from listings); Batch 2 hit it again even harder (Rolls-Royce Spectre needed the price bound raised to ₹950L, EQS needed the range bound raised to 900km, Maybach EQS SUV needed the battery bound raised to 130kWh) but caught and fixed it *before* declaring the batch done, confirming "54 vehicles found" on first load rather than discovering the bug after the fact. Neither typechecking nor the build catches this — only live verification of the listing count does. Expect the two-wheeler bounds (`TWO_WHEELER_FILTER_CONFIG`, currently `priceBounds: [0, 2]` lakh) to need similar attention once Batch 3 adds premium scooters/motorcycles.
5. **Check for `src/lib/data/_research-commercial.ts` and `src/lib/data/_research-two-wheelers.ts`.** These contain real, web-researched vehicle data (original wording, no copied marketing copy, no fabricated prices) that was never reviewed or merged, from a session prior to the batch-based approach. Earmarked for Batches 3 and 5 — spot-check a sample of records against public sources before merging, following the exact record conventions already established in `cars.ts`/`two-wheelers.ts`/`commercial.ts` (see the comments at the top of each `_research-*.ts` file). Delete the staging files once merged.
